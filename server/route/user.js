...
function* createUser() {
  ...
  if (user) {
    var profile = yield profileService.create(user.id, user.get({ plain: true }));
    if (!profile) {
 ...
 *  HTTP/1.1 200 OK
 {
     ...
     "source": "未知",
     "mobile": "13726247339",
     "id": "c16e701f-1b3c-11e6-9a7e-00269e815e18",
     "profile": {
         "id": "ebfec605-223e-11e6-98d0-00269e815e18",
 ...
 *  {
 *      "user": {
 *          "id": "85b0cf16-2241-11e6-8a5b-b0c090570f25",
 *          "name": "xxx",
 *          "email": "aaa@gmail.com",
 *          "mobile": "18510655240",
 *          "status": "ACTIVE",
 ...
 *          "profile": {
 *              "id": "38ceadff-ef07-44bd-ad8f-43467b96acc6",
...
function* getOwnedTopics(id) {
  if (this.session.id != id) {
    throw Exception.create(Exception.Types.Forbidden, '不允许获取他人的话题');
  }
  var topics = yield userService.getOwnedTopics(id);
  if (topics) {
    this.status = 200;
    this.body = topics;
  } else {
    throw Exception.create(Exception.Types.ServerError, '获取话题失败.');
  }
}

function* postTopic(userId) {
  var body = this.request.body;
  if (!body) {
    throw Exception.create(Exception.Types.BadRequest, '缺少必要参数');
  }
  body.ownerId = userId;
  var topic = yield userService.postTopic(body);
  if (!topic) {
    throw Exception.create(Exception.Types.ServerError, '创建话题失败');
  }
  this.body = topic;
}
